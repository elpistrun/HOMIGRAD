local LIB = oop.Reg("lib_event_noself")
...
function LIB:Event_Add(class,name,func,prio)
	...
	if IsValid(self) then self:Event_Construct() end--если убрать проверку то инициализация будет долгой пизда
end
...
function LIB:Construct()
	...
	content:Event_Call("Construct",self)
end