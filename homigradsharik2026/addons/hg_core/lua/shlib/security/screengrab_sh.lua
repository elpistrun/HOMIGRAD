timer.Simple(0,function()
	...
	adminPanel.successRegistry("sg",nil,"rights")
end)
...
if SERVER then
	...
	net.ReceiveBig( "SGRequest", function( _, ply )
		local ent = net.ReadEntity()
		local qual = net.ReadUInt( 32 )
		for k, v in next, player.GetAll() do
			if v.isgrabbing and v ~= ply then
				ply:rtxappend( sg, red, "Error: " .. v:Name() .. "is SGbing " .. v.sg .. ". To reduce server stress, only one SG can be performed at once." )
				return
			end
		end
		if ply.isgrabbing then
			ply:rtxappend( sg.red, "Error: You are already SGbing someone" )
			return
		end
	...
	net.ReceiveBig( "SGInitCallback", function( _, ply )
	...
	net.ReceiveBig( "SGSendPart", function( _, ply )
		local sendto = ply.sg
		local len = net.ReadUInt( 32 )
	...
	net.ReceiveBig( "SGFinished", function( _, ply )
		local _ply = ply.sg
		_ply.parts = nil
		_ply.data = nil
		ply.parts = nil
	...
	net.ReceiveBig( "rtxappend2", function( _, ply )
	...
	net.ReceiveBig( "Progress", function( _, ply )
	...
	hook.Add( "PlayerDisconnected", "SGInterrupt", function( ply )
...
if CLIENT then
	...
	local function cl_rtxappend2( color, text, ply )
    	...
    	local shouldSG = false
    local sgQuality
    local sgPly
    net.ReceiveBig( "StartSG", function()
        shouldSG = true
        sgQuality = net.ReadUInt(32)
        sgPly = net.ReadEntity()
    end)

    hook.Add("PostRender", "SG", function()
        if (!shouldSG) then return end
        shouldSG = false

        cl_rtxappend2( sg.green, "Initializing", sgPly )        
        local function capture( q )
            ...
            cl_rtxappend2( color_white, "Captured " .. len .. " bytes", sgPly )
            local parts = math.ceil( len / split )
            cl_rtxappend2( color_white, parts .. " parts", sgPly )
            local partstab = {}
            for i = 1, parts do
                local min
                local max
            ...
            local i = 1
            timer.Create( "SGSendParts", 0.1, amt, function()
                net.Start( "SGSendPart" )
                    local l = partstab[ i ]:len()
                    net.WriteUInt( l, 32 )
                    net.WriteData( partstab[ i ], l )
                net.SendToServer()
                cl_rtxappend2( Color( 255, 255, 0 ), "Sent " .. i .. STNDRD( i ) .. " part", sgPly )
				net.Start( "Progress" )
	...
	local function DisplayData( str, name )
		...
		if not name then
			main:SetTitle( "SG of " .. LocalPlayer().gfname .. " (" .. string.len( str ) .. " bytes, took " .. elapsedtime .. " seconds)" )
		else
	...
	net.ReceiveBig( "SGInterrupted", function()
	...
	net.ReceiveBig( "SGConfirmation", function()
		local parts = net.ReadUInt( 32 )
		local len = net.ReadUInt( 32 )
		local time = net.ReadFloat()
		local ent = net.ReadEntity()
	...
	net.ReceiveBig( "SendPartBack", function()
		local len = net.ReadUInt( 32 )
		local data = net.ReadData( len )
	...
	function OpenSGMenu()
		...
		main.Paint = function()
			...
			surface.SetTextPos( main:GetWide() / 2 - surface.GetTextSize( "SG Menu" ) / 2, 5 ) 
			surface.SetTextColor( 255, 255, 255, 255 )
			surface.DrawText( "SG Menu" )
		end
		...
		local f = file.Find( "SGs/*.txt", "DATA" )
		files.filetable = f
		...
		files.OnRowRightClick = function( main, line )
			local menu = DermaMenu()
				...
				menu:AddOption( "View Screenshot", function()
					local f = file.Read( "SGs/" .. files:GetLine( line ):GetValue( 1 ), "DATA" )
		...
		net.ReceiveBig( "rtxappend", function()
			local col = net.ReadColor()
			local str = net.ReadString()
		...
		save.DoClick = function()
			if not file.Exists( "SGs", "DATA" ) then
				file.CreateDir( "SGs" )
			end
			...
			file.Write( "SGs/" .. name, text )
			timer.Simple( 1, function()
				if file.Exists( "SGs/" .. name, "DATA" ) then
