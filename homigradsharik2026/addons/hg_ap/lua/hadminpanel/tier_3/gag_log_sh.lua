if CLIENT then
    adminPanel.commandCreate("gag_log_menu",function()
        managerLogGag:OpenMenu()
    end)

    function managerLogGag:CreateSearch(textEntryCaller,panel)
        function textEntryCaller:OnChange()
            managerLogGag.frame.findByCaller = textEntryCaller:GetValue()
        end
        ...
        function textEntryBanned:OnChange()
            managerLogGag.frame.findByBanned = textEntryBanned:GetValue()
        end
    end

    function managerLogGag:CreateInfo(buttCaller,butt,info)
    ...
    function managerLogGag:GetSteamid64Victim(info) return info.content.steamid64 end
    
    function managerLogGag:Request()
        local frame = managerLogGag.frame
...
adminPanel.commandRegistry("gag_log_menu",{},"game",nil,"admin")
adminPanel.commandRegistry("gag_log",{{type = "string",name = "page"},{type = "string",name = "findByAction"},{type = "string",name = "findByCaller"}},"async",nil,"admin")