...
blockTypes["player"] = function( panel, data, color, font )
    ...
    if IsValid( data.ply ) then
        ...
        color = hook.Run("CustomChat_NameColor",data.ply) or color
    end
...
blockTypes["url"] = function( panel, url, _, font )
    ...
    elseif IsValid( CustomChat.lastSpeaker ) then
        canEmbed = hook.Run( "CanEmbedCustomChat", CustomChat.lastSpeaker, url, urlType )
    end

    if TypeID(canEmbed) == TYPE_STRING then return canEmbed end

    if canEmbed ~= nil and CustomChat.IsWhitelisted( url ) then
