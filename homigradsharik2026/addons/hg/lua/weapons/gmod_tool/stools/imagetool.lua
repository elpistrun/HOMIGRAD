...
adminPanel.successRegistry("imagetool",nil,"rights")

if CLIENT then
    CreateClientConVar("imagetool_screamer","0",true,true)
end

-- Левая кнопка мыши
function TOOL:LeftClick()
    if not self:GetOwner():HasSuccess("imagetool") then return false end
    ...
    if client:GetInfoNum("imagetool_screamer",0) == 1 then
        data.isScreamer = true
    end
...
function TOOL.BuildCPanel(CPanel)
    ...
    local Screamer = vgui.Create("DCheckBoxLabel")
    Screamer:SetText("Use Like Screamer")
    Screamer.OnChange = function(_,value)
        RunConsoleCommand("imagetool_screamer",value and 1 or 0)
    end
    CPanel:AddPanel(Screamer)
