local ENT = oop.Reg("sent_simpletimer_sh","base_entity")
...
if CLIENT then
	surface.CreateFont( "xdeti_Font1", { font = "Tahoma", size = 30, weight = 1000, antialias = true, bold = true } )
	surface.CreateFont( "xdeti_Font2", { font = "Tahoma", size = 60, weight = 1000, antialias = true, bold = true } )
	STIMER_ = {
		Name = "",
		Time = 0,
		Timer = 0,
	...
	hook.Add( "HUDPaint", "SimpleTimerHUD", function() local tim, ply = STIMER_ENT, LocalPlayer()
		if STIMER_.LerpAlp > 0 and STIMER_.Timer > 0 then local sta = STIMER_.State
			...
			font = "xdeti_Font1", xalign = TEXT_ALIGN_CENTER,
			...
			font = "xdeti_Font2", xalign = TEXT_ALIGN_CENTER,
			yalign = TEXT_ALIGN_DOWN, color = col }, 1, STIMER_.LerpAlp*255 )
		end
	end )
	hook.Add( "Think", "SimpleTimerThink", function()
		if !IsValid( STIMER_ENT ) then
			if STIMER_.LerpAlp > 0 then STIMER_.LerpAlp = Lerp( 0.025, STIMER_.LerpAlp, 0 ) end
		end
	end )
else
	hook.Add( "PlayerDeathThink", "SimpleTimerDeath", function( ply )
...
function ENT:STimer_Event( num ) if CLIENT or !isnumber( num ) then return end
	...
	elseif num == 4 then game.CleanUpMap( false, { "sent_simpletimer" } )
...
function ENT:SpawnFunction( ply, tr, ClassName )
	...
	ent:SetST_Name( "Simple Timer" ) ent:SetST_Color( Vector( 0, 1, 1 ) ) ent:SetST_Time( 60 )
	...
	ent:SetST_AMission( 0 ) ent:SetST_ATimer( 2 ) return ent
end
function ENT:SetupDataTables()
	...
	if SERVER then
		self:NetworkVarNotify( "ST_State", function( self, var, old, new ) if old == new then return false end
			if WireLib then Wire_TriggerOutput( self, "State", new ) end
			if new == 0 then self:SetColor( Color( 255, 255, 255 ) )
			elseif new == 1 then self:SetColor( Color( 255, 255, 0 ) ) self:STimer_Event( self:GetST_EStart() )
				self:SetST_Timer( CurTime() +self:GetST_Time() )
			elseif new == 2 then self:SetColor( Color( 255, 0, 0 ) ) self:STimer_Event( self:GetST_EEnd() )
			elseif new == 3 then self:SetColor( Color( 0, 255, 0 ) ) self:STimer_Event( self:GetST_EStop() )
...
function ENT:Initialize()
	...
	self:SetCollisionGroup( COLLISION_GROUP_WORLD ) self:DrawShadow( false )
	self:SetST_State( 0 ) self:SetST_Timer( 0 ) self:NetworkVar( "Float", 0, "ST_Timer" )
	if IsValid( STIMER_ENT ) then STIMER_ENT:Remove() end STIMER_ENT = self
	self:SetUseType( SIMPLE_USE ) self:SetST_NextUse( CurTime() +0.5 ) self.SecondTick = 0
...
function ENT:Think()
	if SERVER then local sta, ati, alo = self:GetST_State(), self:GetST_ATimer(), false
		...
		if STIMER_.State != self:GetST_State() then local sta, snd, tex = self:GetST_State(), self:GetST_HSnd(), self:GetST_HNot()
			...
			if sta == 1 then
				if !snd then surface.PlaySound( "ambient/alarms/warningbell1.wav" ) end
				if !tex then chat.AddText( col, STIMER_.Name, Color( 255, 255, 255 ), " started. Timeout: "..math.Round( self:GetST_Timer()-CurTime() ).."s." ) end
			elseif sta == 2 then
				if !snd then surface.PlaySound( "physics/metal/metal_grate_impact_hard1.wav" ) end
				if !tex then chat.AddText( col, STIMER_.Name, Color( 255, 255, 255 ), " expired." ) end
			elseif sta == 3 then
				if !snd then surface.PlaySound( "ambient/levels/canals/windchime2.wav" ) end
			...
			end STIMER_.State = self:GetST_State()
		end
		if STIMER_.State == 1 and !self:GetST_HHud() then STIMER_.LerpAlp = Lerp( 0.05, STIMER_.LerpAlp, 1 ) end
...
function ENT:Use( act )
	if !IsValid( act ) or !act:IsPlayer() or self:GetST_NextUse() > CurTime() then return end
	...
	if sta == 0 then self:SetST_State( 1 ) elseif sta == 1 then self:SetST_State( 3 ) else self:SetST_State( 0 ) end
end
