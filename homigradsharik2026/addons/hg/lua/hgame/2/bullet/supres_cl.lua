if CLIENT then
    ...
    event.Add("PreCalcView","Supperes",function(ply,view)
        ...
        ang[1] = ang[1] + SupressK * 4
        ang[2] = ang[2] + math.cos(CurTime() * 0.015 + addTime) * SupressK * 2
    end)
    ...
    local function PlaySuppressionSound(k)
        ...
        SupressK = math.max(SupressK,k)
    end

    event.Add("DSP","Suppress",function()
        if SupressK > 0.75 or suppressionSound then return 14,false end
    end)
    ...
    hook.Add("RenderScreenspaceEffects","SuppressionEffects",function()
        ...
        if suppressionSound then
            ...
            SupressK = max(SupressK - 1 * FrameTime() / 10,0)
        else
            SupressK = max(SupressK - 1 * FrameTime(),0)
        end

        local k = min(SupressK + max(suppressionSoundTime - time,0) / 3,1)

        DrawColorModify({
            ["$pp_colour_addr"] = 0,
            ["$pp_colour_addg"] = 0,
            ["$pp_colour_addb"] = 0,
            ["$pp_colour_brightness"] = -0.1 * k,
        ...
        ang[1] = ang[1] + math.cos(time) * k / 10
        ang[2] = ang[2] + math.sin(time) * k / 10
    ...
    event.Add("Bullet Crack","CheckSuppression",function(k)
        ...
        angles[1] = angles[1] + math.sin(CurTime() * 0.019 + addTime) * SupressK
        angles[2] = angles[2] + math.cos(CurTime() * 0.019 + addTime) * SupressK
