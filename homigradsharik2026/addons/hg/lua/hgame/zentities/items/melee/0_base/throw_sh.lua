...
function SWEP:ThrowMelee(cmd,typeAttack)
    ...
    if CLIENT then
        bullet:Event_Add("HitEnd","Stick",function(self,traceResult)
            if not traceResult or not traceResult.Hit then return end
            if traceResult.HitSky then return end

            -- Only stick into solid world geometry / non-player entities
            if IsValid(traceResult.Entity) and traceResult.Entity:IsPlayer() then return end

            local sNetworker = self.sNetworker
            if not IsValid(sNetworker) or not sNetworker:IsWeapon() then return end

            net.Start("melee_throw_stick")
                net.WriteUInt(sNetworker:EntIndex(),16)
                net.WriteVector(traceResult.HitPos)
                net.WriteNormal(traceResult.HitNormal)
            net.SendToServer()
        end)
    end
...
if SERVER then
    util.AddNetworkString("melee_throw_stick")

    net.Receive("melee_throw_stick",function()
        local ent = Entity(net.ReadUInt(16))
        local pos = net.ReadVector()
        local normal = net.ReadNormal()

        if not IsValid(ent) or not ent.StickInWall then return end
        if ent.throwStuck then return end

        ent:StickInWall({Hit = true,HitPos = pos,HitNormal = normal})
    end)
end
