local SWEP = oop.Reg("wep_melee_evangelion","wep_melee_base")
if not SWEP then return end

SWEP.PrintName = "Longinus Evangelion"
...
SWEP.Secondary.Force = 300000
SWEP.Secondary.ForceRagdoll = SWEP.Primary.Force * 10
SWEP.Secondary.Throw = "melee_longinus"
...
SWEP.AnimationList["attack_throw"] = {
    ...
    Start = function(self)
        if SERVER then
            ...
            sound.EmitGunShoot(pos,"none","buckshoot",1)

            for i = 1,10 do
                sound.EmitNET(nil,"dwr/bulletcracks/distant/12.ogg",150,1,80,pos,0)
                net.Broadcast()
            end
        end
    end,
...
ammoGame.Reg({
    ...
    bulletInfo = {
        ...
        FlySound = {
            list = {"homigrad/wind/mirrorsedge.wav"},
            volume = 1,
            level = 150,
            pitch = 100
        },
...
function SWEP:HitPost(result,typeAttack,surfaceName)
    ...
    for i = 1,5 do
        surfaceWorld.CreateEffectBullet_Net(pos + Vector(0,math.Rand(-10,10),math.Rand(-10,10)):Rotate(result.HitNormal:Angle()),result.HitNormal,result.Entity,surfaceName,20)
        recipientFilter:AddPVS(result.HitPos)
        net.Send(recipientFilter)
    end

    for i = 1,3 do
        sound.EmitGunShoot(pos,"none","explosive",1)
    end

    for i = 1,3 do
        sound.Emit(nil,"weapons/m67/m67_detonate_dist_03.wav",90,1,100,pos,0)
    end

    timer.Simple(0.1,function()
        for i = 1,3 do
            sound.EmitPanorama(0.7,"weapons/m67/m67_detonate_dist_03.wav",150,1,50 - i * 10,pos,0)
        end
    end)

    for i,ent in pairs(ents.FindInSphere(result.HitPos,400)) do
        if ent:IsPlayer() then FakeDown(ent) end
    end

    Explosive_WreckBuildings(nil,result.HitPos,10,800,true,true)
    Explosive_BlastDoors(nil,result.HitPos,10,700,true)
