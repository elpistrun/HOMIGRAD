...
if CLIENT then
    keyboard.DefaultBindCode("hg_weapon_hands_fight",KEY_R,true)
end

if SERVER then
    concommand.Add("hg_weapon_hands_fight",function(ply)
        if not IsValid(ply) or not ply:Alive() then return end

        local wep = ply:GetActiveWeapon()
        if not IsValid(wep) or wep:GetClass() ~= "weapon_hands" then return end

        wep:SetFightState(not wep:GetFightState())
    end)
end
