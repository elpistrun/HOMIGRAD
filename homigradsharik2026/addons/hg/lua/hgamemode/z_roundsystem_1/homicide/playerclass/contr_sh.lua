...
CLASS.weapons = {"weapon_transmitter","med_band_big","med_kit","med_painkiller","med_adrenaline","weapon_handcuffs","weapon_taser","weapon_hg_flashbang"}
CLASS.main_weapon = {"wep_ar15"}
CLASS.secondary_weapon = {"wep_m9a3"}
...
local outfit = util.JSONToTable('{"09_watch":[],"04_foot":["foot_boots","default"],"06_headmask":["headmask","black"],"08_gloves":["gloves_mechanix","black"],"13_body":["male_07_body","white"],"02_upper":["upper_bomber","black"],"05_hair":["hair_bobcut","black"],"01_top":["top_overshirtlong","black"],"07_scarf":[],"03_lower":["lower_cargo","black"],"11_sunglasses":["sunglasses_aviator","black"],"10_hat":[]}')
...
function CLASS:On()
	...
	armorGame.GiveEntity(self,"vest_thor_crv",{color = color})

	armorGame.GiveEntity(self,"helmet_ops_fast_black",{})
	armorGame.GiveEntity(self,"mask_balistic",{})
	armorGame.GiveEntity(self,"headset_m32",{})

	local subColor2 = Color(color.r * 2,color.g * 2,color.b * 2)
	local subColor3 = Color(color.r * 3,color.g * 3,color.b * 3)

	armorGame.GiveEntity(self,"updump_thunderbolt",{color = subColor2})
	
	for i,weapon in pairs(CLASS.weapons or empty) do self:Give(weapon) end

	level_tdm.GiveSwep(self,CLASS.main_weapon)
	level_tdm.GiveSwep(self,CLASS.secondary_weapon)
	...
	local _,model = table.Random(outfitPlayer.config_model)

	self:SetOutfit({slots = outfit,model = model})
end
...
function CLASS:DamagePlayer(hitGroup,dmgInfo,rag)
	...
	for x,list in pairs(interface) do
		...
		for y,snd in pairs(list) do
			...
			function button:Paint(w,h)
				...
				draw.SimpleText(snd[2],"H.18",w / 2,h / 2,white2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			end
