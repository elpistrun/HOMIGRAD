local MUTATOR = Mutator_Get("water")
if not MUTATOR then return end

MUTATOR.ResetWithCleanUp = true

if SERVER then
    util.AddNetworkString("water in")
    util.AddNetworkString("water out")
...
net.Receive("water out",function()
    local pos = net.ReadVector()
    local size = tonumber(net.ReadString())
