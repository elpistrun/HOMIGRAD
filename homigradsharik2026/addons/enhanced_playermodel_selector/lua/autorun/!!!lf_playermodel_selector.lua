...
local blacklistMdl = {
	["models/breach173.mdl"] = true,
	["models/player/scp096.mdl"] = true,
	...
	["models/player/brenton/ron_fbi/models/brenton.mdl"] = true,
	["models/player/open season/boog.mdl"] = true,
	["models/player/creepergirl/sour_creepergirl_player.mdl"] = true,
	["models/player/cmbturretgirl.mdl"] = true,

	["models/player/minion/minion1/minion.mdl"] = true,
	["models/player/minion/minion2/minion2.mdl"] = true,
	["models/player/minion/minion3/minion3.mdl"] = true,
	["models/player/minion/minion4/minion4.mdl"] = true,
	["models/player/minion/minion5/minion5.mdl"] = true,
	["models/player/minion/minion6/minion6.mdl"] = true,

	["models/player/turretgirl.mdl"] = true,
	["models/duskhd/horror/horrorhd_pm.mdl"] = true,

	["models/vedatys/orangutan.mdl"] = true,
	["models/player/zelenium.mdl"] = true,
...
convars["sv_playermodel_selector_flexes"]		= 1
convars["sv_playermodel_selector_limit"]		= 0
convars["sv_playermodel_selector_debug"]		= 0
for cvar, def in pairs( convars ) do
	CreateConVar( cvar,	def, flag )
	RunConsoleCommand(cvar,def)
end

flag = nil

if SERVER then
	util.AddNetworkString("PlayerSetModel")
else
	net.Receive("PlayerSetModel",function()
		...
		for i = 0,ply:GetBoneCount() - 1 do
			ply:ManipulateBoneScale(i,vecFull)
		end
	end)
end
...
local function Allowed( ply )
	if hook.Run("CanChangePlayerModel",ply) then
		return true
	else
		return false
	end
...
function UpdatePlayerModel( ply )
	if Allowed( ply ) then
		...
		local mdlpath = string.lower(player_manager.TranslatePlayerModel( mdlname ))
	
		if not ply:IsAdmin() and blacklistMdl[mdlpath] then mdlpath = "models/player/kleiner.mdl" end
...
net.Receive("lf_playermodel_update", function( len, ply )
	if ply:IsValid() and ply:IsPlayer() and ( ply:IsAdmin() or hook.Run("PlayerModel Apply",ply) == true ) then
...
function Menu.Setup()
	...
	Menu.ApplyButton:SetEnabled(true)-- LocalPlayer():IsAdmin() or GetConVar( "sv_playermodel_selector_instantly" ):GetBool() )
	...
	Menu.Right:SetSize( 430, 0 )
		...
		Menu.Right:AddSheet( "Model", modeltab, "icon16/user.png" )
			...
			ModelList.OnRowSelected = function()
				...
				--if blacklistMdl[name] then LocalPlayer():Notify("Запрещено",NOTIFY_ERROR,3) return end
			...
			function Menu.ModelPopulate()
				...
				for name, model in SortedPairs( AllModels ) do
					...
					if IsInFilter( name ) then
						...
						local icon = ModelIconLayout:Add( oop.CreatePanel("v_spawnicon") )
						icon:SetSize( 64, 64 )
	...
	function Menu.RebuildBodygroupTab()
		...
		for k = 0, mdl.Entity:GetNumBodyGroups() - 1 do
			...
			bgroup:Dock( TOP )
			bgroup:SetText( "bodygroup " .. k .. " " .. Menu.MakeNiceName( mdl.Entity:GetBodygroupName( k ) ) )
			bgroup:SetDark( true )
			bgroup:SetTall( 50 )
		...
		if GetConVar( "sv_playermodel_selector_flexes" ):GetBool() and GetConVar( "cl_playermodel_selector_unlockflexes" ):GetBool() then
			...
			for k = 0, mdl.Entity:GetFlexNum() - 1 do
				...
				flex:Dock( TOP )
				flex:SetText( "flex " .. Menu.MakeNiceName( mdl.Entity:GetFlexName( k ) ) )
				flex:SetDark( true )
				flex:SetTall( 30 )
				flex:SetDecimals( 2 )
				flex.type = "flex"
