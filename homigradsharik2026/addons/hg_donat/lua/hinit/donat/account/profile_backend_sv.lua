...
local function LoadFromDB(sid)
    if not sql.TableExists("hg_profile") then return end
    local row = sql.QueryRow("SELECT data FROM hg_profile WHERE steamid64 = " .. sql.SQLStr(sid))
    if row and row.data then return util.JSONToTable(row.data) or {} end
end
hook.Add("Initialize","HG Profile DB",function()
    if not sql.TableExists("hg_profile") then sql.Query("CREATE TABLE hg_profile (steamid64 TEXT PRIMARY KEY, data TEXT)") end
    local fileData = util.JSONToTable(file.Read(dataPath,"DATA") or "") or {}
    for sid, data in pairs(fileData) do
        if not sql.QueryValue("SELECT steamid64 FROM hg_profile WHERE steamid64 = " .. sql.SQLStr(sid)) then
            sql.Query("INSERT INTO hg_profile (steamid64, data) VALUES (" .. sql.SQLStr(sid) .. ", " .. sql.SQLStr(util.TableToJSON(data) or "{}") .. ")")
        end
    end
end)

if file.Exists(dataPath,"DATA") then
    local fileProfiles = util.JSONToTable(file.Read(dataPath,"DATA") or "") or {}
    for sid, data in pairs(fileProfiles) do profiles[sid] = data end
    local dbRows = sql.Query("SELECT steamid64, data FROM hg_profile")
    if dbRows then
        for _, row in ipairs(dbRows) do
            local dbData = util.JSONToTable(row.data or "") or {}
            profiles[row.steamid64] = table.Merge(profiles[row.steamid64] or {}, dbData)
        end
    end
end
...
local function Save()
    ...
    for sid, data in pairs(profiles) do
        sql.Query("REPLACE INTO hg_profile (steamid64, data) VALUES (" .. sql.SQLStr(sid) .. ", " .. sql.SQLStr(util.TableToJSON(data) or "{}") .. ")")
    end
end
