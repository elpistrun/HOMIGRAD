...
function FandigDoor_onUp(pl, Ent)
	if IsValid(Ent) then
...
numpad.Register("Fading Door onUp", function(ply,ent)
	if roundActiveName == "level_construct" then return end

	FandigDoor_onUp(ply,ent)
end)

function FandigDoor_onDown(pl, Ent)
	if IsValid(Ent) then
		local Activate = true
		if Ent.fadeToggle then
			if Ent.fadeReversed then
...
numpad.Register("Fading Door onDown", function(ply,ent)
	if roundActiveName == "level_construct" then return end

	FandigDoor_onDown(ply,ent)
end)
...
FandingDoors = FandingDoors or {}

local function onRemove(self)
	if self.fadeKey then
		local list = FandingDoors[self.fadeKey .. self.COwner]
		if list then list[self:EntIndex()] = nil end
	end
...
function createFandingDoor(pl, Ent, stuff)
	...
	local name = stuff.key .. Ent.COwner
	FandingDoors[name] = FandingDoors[name] or {}
	FandingDoors[name][Ent] = true

	return true
end

duplicator.RegisterEntityModifier("Fading Door", createFandingDoor)
hook.Add("Initialize", "FadingDoor1", function() duplicator.RegisterEntityModifier("Fading Door", createFandingDoor) end)	-- No overwrite.

if !FadingDoor then
	local function legacy(pl, Ent, data)
		return createFandingDoor(pl, Ent, {
			key					= data.Key,
			toggle				= data.Toggle,
