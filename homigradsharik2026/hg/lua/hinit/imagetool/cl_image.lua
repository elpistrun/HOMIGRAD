...
function ImageTool:LoadingURL(data)
    ...
    end*/--и ты ещё на какие-то права притендуешь?

    local chache = self:FindInCache(uniqueID)
    if chache then return chache end
...
function ImageTool:Start3D2D(data)
    ...
    local scale = (tonumber(data.scale) or 7) / 100
    ...
    -- Рисуем
    cam.Start3D2D(data.position + Vector(0,0,0.7):Rotate(data.angles), data.angles, scale)
        w = w * 4
        h = h * 4
        ...
        render.PushFilterMag(TEXFILTER.ANISOTROPIC)
            ...
            if not data.isImageTool and data.ownerSteamID and data.position:Distance(EyePos()) <= 128 then
                draw.SimpleText(data.ownerSteamID,"HS.45",0,h/2 + 30,nil,TEXT_ALIGN_CENTER)
            end
        render.PopFilterMag()
        render.PopFilterMin()
    cam.End3D2D()
